...
class FreqtradeBot(LoggingMixin):
    ...
    def check_and_call_adjust_trade_position(self, trade: Trade):
        ...
        min_entry_stake = self.exchange.get_min_pair_stake_amount(
            trade.pair, current_entry_rate, self.strategy.stoploss
        )
    ...
    def execute_entry(
        ...
        *,
        ...
        if trade is None:
            ...
            logger.warning("This log is by Bilal")
            logger.warning(trade)
            # logger.warning("base_currency "+ base_currency)
            # logger.warning("Stake Currency "+ self.config['stake_currency'])
            # logger.warning("stake_amount "+ stake_amount)
            # logger.warning("amount "+ amount)
            # logger.warning("amount_requested "+ amount_requested)
            # logger.warning("fee "+ fee)
            # logger.warning("open_rate "+ enter_limit_filled_price)
            # logger.warning("open_rate_requested"+enter_limit_requested)
            ...
            logger.warning(tradeObject)
            base_url = "http://localhost:3000/external-exchanges/binance/createTrade"
            # Get base UI Repo path

            resp = requests.post(base_url, json=tradeObject)
    ...
    def create_stoploss_order(self, trade: Trade, stop_price: float) -> bool:
        ...
        try:
            ...
            self.execute_trade_exit(
                trade, stop_price, exit_check=ExitCheckTuple(exit_type=ExitType.EMERGENCY_EXIT)
            )
    ...
    def handle_stoploss_on_exchange(self, trade: Trade) -> bool:
        ...
        if not stoploss_order:
            stoploss = (
                ...
                if self.edge
                else trade.stop_loss_pct / trade.leverage
            )
            if trade.is_short:
